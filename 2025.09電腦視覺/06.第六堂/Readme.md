...
| 類型                                                | 說明             | 數學形式                                                                                  |
| ------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------- |
| **平移（Translation）**                               | 影像在x、y方向上移動    | [ x' = x + t_x, ; y' = y + t_y ]                                                      |
| **縮放（Scaling）**                                   | 放大或縮小影像        | [ x' = s_x \cdot x, ; y' = s_y \cdot y ]                                              |
| **旋轉（Rotation）**                                  | 以原點或特定中心旋轉     | [ x' = x \cos\theta - y \sin\theta, ; y' = x \sin\theta + y \cos\theta ]              |
| **翻轉（Reflection）**                                | 水平或垂直鏡像        | 水平：( x' = -x )；垂直：( y' = -y )                                                         |
| **仿射變換（Affine Transformation）**                   | 維持平行線但可改變角度與比例 | [ \begin{bmatrix} x' \ y' \end{bmatrix} = A \begin{bmatrix} x \ y \end{bmatrix} + t ] |
